...
/var/nq_chunk_global/nq_chunk = new
	...
	proc
		...
		WriteAtom(atom/atom, savefile/savefile)
			...
			if (istype(atom, /atom))
				...
				if (t && !(t in src.turfs))
					throw EXCEPTION("Object \ref[atom] contains a reference to turf [t.x],[t.y],[t.z], but it doesn't exist in this chunk.")
					...
					if (nq_chunk.IsSaved(atom, v))
						if (istype(atom.vars[v], /atom))
							if (isturf(atom.vars[v]))
								...
								if (t in turfs)
									savefile[v] << "[nq_chunk.SEP_TURF][t.x - x1 + 1],[t.y - y1 + 1],[t.z - z1 + 1]"
								else
									throw EXCEPTION("Object \ref[atom]'s [v] variable contains a reference to turf [t.x],[t.y],[t.z], but it doesn't exist in this chunk.")
							else
